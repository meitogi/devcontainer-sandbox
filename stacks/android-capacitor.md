# Capacitor Android stack — project Dockerfile blocks

Capacitor plugin toolchain (OpenJDK 17 + Kotlin + multi-API `android.jar`
matrix + Coursier + Capacitor SDK + common AndroidX / kotlinx jars with
transitives resolved) on top of `ghcr.io/meitogi/devcontainer-claude-code`.
There is no published Capacitor image : per the v1 architecture decision,
non-Node stacks are **project Dockerfiles** deriving from the single Node 24
base.
...
## 2. Capacitor Android toolchain (Debian OpenJDK 17)

Continues as `USER root` from the preamble ; ends by dropping back to `node`.

```dockerfile
# Everything below is variant-specific (JDK, Kotlin, Android jars, Capacitor
# SDK + AndroidX/kotlinx transitives, helpers). Stays USER root from the
# firewall block above — no explicit switch needed.
