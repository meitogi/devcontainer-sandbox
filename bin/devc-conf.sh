...
ports_file() {
  ...
  if [ -f "$dir/ports.txt" ]; then
    ...
    [ -f "$dir/direct-tcp-allow.txt" ] &&
      echo "⚠️  both ports.txt and direct-tcp-allow.txt exist — reading ports.txt, ignoring the old one" >&2
    printf '%s' "$dir/ports.txt"
  elif [ -f "$dir/direct-tcp-allow.txt" ]; then
    echo "⚠️  direct-tcp-allow.txt is deprecated — rename it to ports.txt" >&2
